...
from ..modules.commons.layers import ConvPositionEmbed, ConvTranspose1d, EmbeddingTable, Linear
from ..modules.commons.mask import compute_random_span_mask, get_mask_from_lengths, mask_data
from ..modules.commons.ops import eval_decorator
from ..modules.dit.modules import FinalLayer, TimestepEmbedder
...
class Token2latentFlowMatching(BaseModel):
    ...
    def forward(
        ...
        span_mask = compute_random_span_mask(
            target, mask_ratio_range=(self.bert_maskrate0, self.bert_maskrate1), x_lens=target_lens, tail_mask=True
        )  # [b,t,d]
        ...
        res = {"pred": outputs, "target": ut, "pred_lens": target_lens, "cond_mask": span_mask.unsqueeze(2)}
        return res
...
class Token2latentFlowMatchingWithEmbed(Token2latentFlowMatching):
    ...
    def forward(
        ...
        return super().forward(
            token=token, token_lens=token_lens, target=target, target_lens=target_lens, spkr_embed=spkr_embed
        )
