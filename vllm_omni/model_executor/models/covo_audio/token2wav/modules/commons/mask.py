...
def random_masking(mask, mask_prob, ignore_first=True):
    ...
    new_mask = ~torch.zeros(mask.shape, device=mask.device).scatter(1, indices, 1.0).bool()
    return new_mask


def get_mask_from_lengths(lengths, max_len=None, r=1, random_mask=0.0):
    if max_len is None:
...
def compute_random_span_mask(
    x, mask_ratio_range: tuple = (0.5, 1.0), x_lens=None, tail_mask: bool = False, min_span: int = 10
):
    """
    args:
