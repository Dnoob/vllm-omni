...
If you are running on GPUs with less VRAM (e.g. 2x 48GB), use the thinker_tp2 config which splits the Thinker stage across both GPUs with tensor_parallel_size=2

```bash
vllm serve Qwen/Qwen3-Omni-30B-A3B-Instruct --omni --port 8091 --stage-configs-path vllm_omni/model_executor/stage_configs/qwen3_omni_moe_thinker_tp2.yaml
```
